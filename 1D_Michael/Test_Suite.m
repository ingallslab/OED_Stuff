...
numExp = 20;
numTrials = 200;
datasrc = 'SSA_Linspace';
Omega = 90;
options = struct('DataSource',datasrc,'SystemSize',Omega); %SSA_Linspace, Norm_Linspace_LNA

fits = generateFits(numFits,numExp,numTrials,options);

disp(fits);

dlmwrite(strcat('./1D_Michael/Data/',datasrc,'_',num2str(Omega),'.txt'),fits,'\t');
